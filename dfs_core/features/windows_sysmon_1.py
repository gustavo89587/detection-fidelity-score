...
def extract_sysmon1_context(event: Dict[str, Any]) -> Sysmon1Context:
    ...
    hashes      = _get(event, "winlog.event_data.Hashes") or _get(event, "process.hash.sha256")
    ...
    suspicious_parent_chain = (
        isinstance(parent_s, str) and isinstance(image_s, str)
    ...
    return Sysmon1Context(
        has_command_line=has_command_line, has_parent_image=has_parent_image,
        has_parent_command_line=has_parent_command_line, has_user=has_user,
        has_process_image=has_process_image, has_hashes=has_hashes,
        has_integrity_level=has_integrity_level, has_logon_id=has_logon_id,
        host_present=host_present, looks_encoded_ps=looks_encoded_ps,
...
def sysmon1_to_inputs_and_flags(event: Dict[str, Any]) -> Tuple[DFSInputs, Dict[str, bool]]:
    ...
    telemetry_fields = [
        ctx.host_present, ctx.has_process_image, ctx.has_command_line,
        ctx.has_parent_image, ctx.has_parent_command_line, ctx.has_user,
        ctx.has_hashes, ctx.has_integrity_level, ctx.has_logon_id,
    ]
    ...
    b = 0.45
    if ctx.has_parent_image:        b += 0.18
    if ctx.has_parent_command_line: b += 0.07
    if ctx.has_user:                b += 0.15
    if ctx.has_logon_id:            b += 0.08
    if ctx.has_integrity_level:     b += 0.05
    if ctx.suspicious_parent_chain: b += 0.02
    b = max(0.0, min(1.0, b))
...
def extract(event, policy=None):
    return sysmon1_to_inputs_and_flags(event)
